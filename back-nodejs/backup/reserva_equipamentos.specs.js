...
Given(
  ...
  async function (equipamentoNome, data) {
    // Cria o equipamento usando a rota POST
    let response = await request(server).post("/equipamentos").send({
    ...
    const equipamentoId = response.body.data._id;

    // Recupera o equipamento criado usando a rota GET
    response = await request(server).get(`/equipamentos/${equipamentoId}`);

    if (response.status !== 200) {
      throw new Error("Falha ao recuperar equipamento");
    }
...
When(
  ...
  async function (equipamentoNome, data) {
    const equipamento = await EquipSala.findOne({ nome: equipamentoNome });
...
Then(
  ...
  async function (equipamentoNome, data) {
    ...
    const equipamento = await Reserva.findOne({ equipSalaId: equipSalaId });
